...
[role="code_command hotspot=mpRetry hotspot=getPropertiesForHost", subs="quotes"]
----
...
The above command returns 3, meaning that a total of 3 requests are made to the `system` service. 
Altough the `system` service is available and your single request to the service is successful with a `200` response code, 
the above command returns 3, instead of 1.
This is because Istio by default will retry 2 times to resolve any issues with a `503` response code. 
So, there were a total of 3 requests made to the `system` service, 
including the initial request and the 2 retries that were made after the initial request.
...
The above command returns 12, since there were a total of 12 requests made to the `system` service.
However, you enabled MicroProfile to retry a maximum of 3 times, and hence the total number of requests to the `system` service should be 4.
This is not the case because the number of retries set by MicroProfile is multiplied by Istio's default number of retries.
...
Istio would have sent a total of 3 requests, including the initial request and the 2 default retries. 
The 3 default requests by Istio are multiplied by the 4 requests by MicroProfile, causing a total of 12 requests.
...
retry requests to the `system` service a maximum of 4 times, when the request returns any `5xx` response code. 
...
[role=command]
```
kubectl replace --force -f istio.yaml
...
However, you enabled Istio to retry a maximum of 4 times, and hence the total number of requests to the `system` service should be 5.
This is not the case because the number of retries set by MicroProfile conflicts with the number of retries set by Istio.
The number of retries set by MicroProfile is multiplied by Istio's default number of retries which is multiplied by the
number of retries that you set in Istio.

Individually, Istio by default would have sent a total of 3 requests, and MicroProfile would have sent a total of 4 requests.
Without MicroProfile and the default Istio requests, Istio would have sent a total of 5 requests because of the retries 
that you enabled in the [hotspot file=0]`traffic.yaml` file. 
The 3 default requests by Istio are multiplied by the 4 requests by MicroProfile and the 5 new requests by Istio, causing a total of 60 requests.

To prevent conflicts between MicroProfile and Istio Fault Tolerance, you will disable some of MicroProfile's capabilities, including Retry, 
to have your `system` service retry the exact number of times indicated in Istio's Retry policy. 
...
and Istio to perform 4 retries, however you noticed using `kubectl logs` that when both retry policies are enabled, 
a total of 60 requests were made to the `system` service.
...
Istio by default would have sent a total of 3 requests, and Istio would have sent 5 requests because of the retries 
that you enabled in the [hotspot]`traffic.yaml` file. 
The 3 default requests by Istio are multiplied by the 5 new requests by Istio, causing a total of 15 requests.
...
[role="code_command hotspot=fallbackMethod hotspot=fallback hotspot=getPropertiesForHost", subs="quotes"]
----
#Update the `InventoryResource.java` file.#
`inventory/src/main/java/io/openliberty/guides/inventory/InventoryResource.java`
----
...
The above command returns 15, indicating that a total of 15 requests are made to the `system` service.
You enabled Istio to retry a maximum of 5 times, which is multiplied by the 3 default requests by Istio,
and hence the total number of requests to the `system` service is 15.
