...
and the [hotspot=getProperties file=0]`getProperties()` in [hotspot file=0]`InventoryResource.java` throws a `ProcessingException`. 
...
to retry the service request a maximum of 3 times, only when an `ProcessingException` occurs.
