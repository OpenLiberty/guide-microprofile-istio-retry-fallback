...
The `Host` header of your `system` service and `inventory` service has a value of `system.example.com` and `inventory.example.com`, respectively. 
...
specified MicroProfile annotations, including `@Retry`, won't take effect and only keep `@Fallback` available to use.
