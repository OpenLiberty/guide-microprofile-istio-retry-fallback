...
Next, the [hotspot file=2]`inventory/src/main/java/io/openliberty/guides/inventory/client/SystemClient.java` file
...
The `system` service as described in the [hotspot file=3]`SystemResource.java` file calls the [hotspot=throwException file=3]`getPropertiesThrowException()`
function which throws an `IOException`, as a 500 status code, to simulate that the system is unavailable.
