...
Hence, the 3 requests from the `system` service are multiplied by the 3 requests from the `inventory` service and multiplied by the 4 MicroProfile requests.
...
The `[system-pod-name]` will be regenerated and is different than the previous one you have been using.
