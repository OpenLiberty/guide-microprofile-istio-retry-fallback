...
:page-seo-description: A tutorial on MicroProfile Fault Tolerance integrates with Istio Fault Tolerance and an example on how to develop fault-tolerance microservices using Istio's Retry policy and MicroProfile Fault Tolerance's Fallback mechanism.
...
Explore how to manage the impact of failures using MicroProfile and Istio Fault Tolerance by adding retry and fallback behaviors to microservices.
...
Later, you will disable some of MicroProfile's capabilities, to have your `system` service retry with only Istio's Retry policy. 
...
The `MP_Fault_Tolerance_NonFallback_Enabled` config property must be defined in a ConfigMap.
...
Deploy your microservices again to turn off all MicroProfile capabilities, except fallback:
