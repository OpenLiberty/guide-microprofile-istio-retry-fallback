...
----
[role="edit_command_text"]
Add the [hotspot=32-35 file=0]`retries` field under the [hotspot=27-34 file=0]`route` specification in the [hotspot file=0]`traffic.yaml` file. 
This tells Istio to retry requests a maximum of 4 times when the request returns any `5xx` response code. 

The [hotspot=33 file=0]`attempts` field is required in the configuration of the 
...
[hotspot=34 file=0]`retryOn` field. Since your paused `system` service responds with a `503` response code, 
you set [hotspot=34 file=0]`retryOn` to be `5xx`. 
...
can also be specified in [hotspot=34 file=0]`retryOn`. Optionally, the `perTryTimeout` field can be added to Istio's Retry policy 
