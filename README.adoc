...
Since the the `system` service is unavailable, the request returns a `503` response code.
...
The above command returns 37, since there was a total of 37 requests made to the `system` service.
...
----
[role="edit_command_text"]
Add the [hotspot=istioRetry file=0]`retries` field under the [hotspot=27-35 file=0]`route` specification in the [hotspot file=0]`traffic.yaml` file. 
This tells Istio to retry requests a maximum of 4 times when the request returns any `5xx` response code. 

The [hotspot=34 file=0]`attempts` field is required in the configuration of the 
...
[hotspot=35 file=0]`retryOn` field. Since your paused `system` service responds with a `503` response code, 
you set [hotspot=35 file=0]`retryOn` to be `5xx`. 
...
can also be specified in [hotspot=35 file=0]`retryOn`. Optionally, the `perTryTimeout` field can be added to Istio's Retry policy 
...
Since the `system` service is unavailable, the request still returns a `503` response code.
...
----
[role="edit_command_text"]
Add a [hotspot=95-102 file=0]`ConfigMap` into the [hotspot file=0]`services.yaml` file, and 
set the [hotspot=102 file=0]`MP_Fault_Tolerance_NonFallback_Enabled` config property to false. 
Add the [hotspot=invConfig file=0]`envFrom` field 
to inject the [hotspot=95-102 file=0]`ConfigMap` with the [hotspot=102 file=0]`MP_Fault_Tolerance_NonFallback_Enabled` property into your pods. 

The [hotspot=configName file=0]`name` of the [hotspot=95-102 file=0]`ConfigMap`, which is [hotspot=configName file=0]`inventory-config`, 
...
[hotspot=102 file=0]`MP_Fault_Tolerance_NonFallback_Enabled` config property, the fallback policy is still available.
...
Since all of these requests failed, the [hotspot=fallbackMethod file=1]`getPropertiesFallback()` fallback method is called. 
