...
:page-seo-title: Building resilient microservices using MicroProfile and Istio Fault Tolerance
:page-seo-description: A tutorial on MicroProfile Fault Tolerance integrated with Istio Fault Tolerance and an example on how to develop fault-tolerance microservices using Istio's Retry policy and MicroProfile Fault Tolerance's Fallback mechanism.
...
Explore how to manage the impact of failures using MicroProfile and Istio Fault Tolerance by adding retry and fallback behaviours to microservices.
...
Microservices that are created using Eclipse MicroProfile can be freely deployed in a service mesh architecture to reduce the complexity 
...
Fault tolerance leverages different strategies for building robust behaviour to cope with unexpected failures.
...
with your `system` service, and try to send requests to the service. When the `system` pod is paused, requests to the service return a `503` status code, 
...
To retry the requests to your `system` service after an `WebApplicationException` has occurred, add the `@Retry` annotation.
...
to retry the service request a maximum of 3 times, only when an [hotspot=webApplicationException file=0]`WebApplicationException` occurs.
...
You will see that the above command returns 1, meaning that 1 request is made to the `system` service:
...
The request returns a `503` response code, as the `system` service is unavailable.
However, the request retried several times as per what is specified by the MicroProfile `@Retry` annotation.
...
The above command returns 37, since there were a total of 37 requests made to the `system` service.
...
Hence, the 3 requests from `system` service and the 3 requests from the `inventory` service are multiplied by the 4 MicroProfile requests.
...
to allow Istio to retry requests a maximum of 4 times, when the request returns any `5xx` response code. 

The [hotspot=attempts file=0]`attempts` field is required in the configuration of the 
https://istio.io/docs/reference/config/networking/v1alpha3/virtual-service/#HTTPRetry[Istio Retry^] policy, and it indicates the
maximum number of retries that will be attempted for a given request. To retry a request on specific conditions, the
[hotspot=retryOn file=0]`retryOn` field is used. Since your paused `system` service responds with a `503` response code, 
...
After configuring the numnber of retries performed by Istio for your microservices, deploy your microservices again: 
...
You will see that the request still returns a `503` response code, as the `system` service is unavailable.
...
The 3 default Istio requests for the `system` service and the 5 requests for the `inventory` service
that you enabled in the [hotspot file=0]`traffic.yaml` file, are multiplied by the 4 requests sent by MicroProfile.

Next, you will disable some MicroProfile Fault Tolerance capabilities, to have your `system` service retry with only Istio's Retry policy. 
...
When we have both MicroProfile and Istio Fault Tolerance capabilities enabled, there is a compounding effect that may be unexpected. 
...
In the case that you want to use Istio as your service mesh and its fault tolerance capabilities, 
you can turn off MicroProfile Fault Tolerance via a property to avoid any overlap in behaviours.

MicroProfile Fault Tolerance offers a config property `MP_Fault_Tolerance_NonFallback_Enabled` that disables all 
MicroProfile Fault Tolerance capabilities except fallback. If `MP_Fault_Tolerance_NonFallback_Enabled` is set to false, all of the 
behaviours specified by the MicroProfile Fault Tolerance annotations, including `@Retry`, won't take effect except for the `@Fallback` behaviour.
...
used by the pod's containers. The environment variables are defined in the pod's specification using the `envFrom` field. To learn more about ConfigMaps,
...
Since retrying the requests to the `system` service still does not give you success, you need a "fall back" plan. 
You will create a fallback method as an alternative solution when the strategy to retry requests to the `system` service have failed.
