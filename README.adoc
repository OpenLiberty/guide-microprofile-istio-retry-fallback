...
Before rebuilding your application, update the version of the microservices to `2.0-SNAPSHOT`:
...
----
[role="edit_command_text"]
Update the `inventory` service's [hotspot=version file=2]`version` from `1.0-SNAPSHOT` to `2.0-SNAPSHOT`.
...
[role='command']
```
docker build -t inventory:2.0-SNAPSHOT inventory/.
```

Before redeploying your microservices, you must update your `inventory` microservice's deployment manifest in `services.yaml`
...
----
services.yaml
[source, yaml, linenums, role='code_column hide_tags=copyright']
----
include::finish/services.yaml[]
----
[role="edit_command_text"]
Update the `inventory-deployment` deployment's [hotspot=version file=2]`image` from `1.0-SNAPSHOT` to `2.0-SNAPSHOT`.
