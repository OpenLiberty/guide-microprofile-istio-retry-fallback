...
However, you enabled MicroProfile to retry a maximum of 3 times, and hence the total number of requests to the `system` service, 
including the initial request, should be 4. 
This is not the case because Istio by default will retry 2 times to resolve any issues with a `503` response code. 
Istio's default number of retries are then multiplied by the number of retries set by MicroProfile.

Without MicroProfile, Istio would have sent a total of 3 requests, including the initial request and the 2 default retries. 
Individually, MicroProfile would have sent a total of 4 requests, including the initial request and 3 retries. 
...
Pause the `system` service pod to simulate that the service is unavailable. Replace `[system-pod-name]` with the pod name associated with
your `system` service, which you can see by running the `kubectl get pods` command.
...
that you enabled in the [hotspot file=2]`traffic.yaml` file. 
